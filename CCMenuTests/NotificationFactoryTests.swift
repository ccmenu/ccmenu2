...
class NotificationFactoryTests: XCTestCase {
    ...
    func testDoesntCreateStartNotificationIfDefaultIsFalse() throws {
        ...
        let previous = PipelineStatus(activity: .sleeping)
    ...
    func testCreatesStartNotificationWithSuccessfulLastBuildWithDurationThatNeedsRounding() throws {
        defaults.set(true, forKey: DefaultsKey.key(forNotification: .started))
        pipeline.status = PipelineStatus(activity: .building, lastBuild: Build(result: .success, duration: 65))
        let previous = PipelineStatus(activity: .sleeping)
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testCreatesStartNotificationWithFailedLastBuildWithDurationThatCanBeCollapsed() throws {
        defaults.set(true, forKey: DefaultsKey.key(forNotification: .started))
        pipeline.status = PipelineStatus(activity: .building, lastBuild: Build(result: .failure, duration: 62*60))
        let previous = PipelineStatus(activity: .sleeping)
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testCreatesStartNotificationWithUnknownLastBuildWithDurationThatNeedsHoursAndMinutes() throws {
        defaults.set(true, forKey: DefaultsKey.key(forNotification: .started))
        pipeline.status = PipelineStatus(activity: .building, lastBuild: Build(result: .unknown, duration: 7500))
        let previous = PipelineStatus(activity: .sleeping)
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testAddsWebUrlToUserInfoToStartNotification() throws {
        defaults.set(true, forKey: DefaultsKey.key(forNotification: .started))
        pipeline.status = PipelineStatus(activity: .building, lastBuild: Build(result: .success), webUrl: "http://localhost/connectfour")
        let previous = PipelineStatus(activity: .sleeping, lastBuild: Build(result: .other))
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testDoesntCreateCompletionNotificationIfDefaultIsFalse() throws {
        ...
        pipeline.status = PipelineStatus(activity: .sleeping, lastBuild: Build(result: .success))
        let previous = PipelineStatus(activity: .building)
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testCreatesCompletionNotificationForSuccessfulBuildWithDuration() throws {
        ...
        pipeline.status = PipelineStatus(activity: .sleeping, lastBuild: Build(result: .success, duration: 300))
        let previous = PipelineStatus(activity: .building)
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testCreatesCompletionNotificationForSuccessfulBuildFollowingFailedBuild() throws {
        ...
        pipeline.status = PipelineStatus(activity: .sleeping, lastBuild: Build(result: .success))
        let previous = PipelineStatus(activity: .building, lastBuild: Build(result: .failure))
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testCreatesCompletionNotificationForFailedBuildWithTime() throws {
        ...
        pipeline.status = PipelineStatus(activity: .sleeping, lastBuild: Build(result: .failure, duration: 300))
        let previous = PipelineStatus(activity: .building)
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testCreatesCompletionNotificationForFailedBuildFollowingFailedBuild() throws {
        ...
        pipeline.status = PipelineStatus(activity: .sleeping, lastBuild: Build(result: .failure))
        let previous = PipelineStatus(activity: .building, lastBuild: Build(result: .failure))
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testCreatesCompletionNotificationForOtherBuildResult() throws {
        ...
        pipeline.status = PipelineStatus(activity: .sleeping, lastBuild: Build(result: .other))
        let previous = PipelineStatus(activity: .building)
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
    ...
    func testAddsWebUrlToUserInfoToCompletionNotification() throws {
        defaults.set(true, forKey: DefaultsKey.key(forNotification: .wasSuccessful))
        pipeline.status = PipelineStatus(activity: .sleeping, lastBuild: Build(result: .success), webUrl: "http://localhost/connectfour")
        let previous = PipelineStatus(activity: .building, lastBuild: Build(result: .other))
        let change = StatusChange(pipeline: pipeline, previousStatus: previous)
